...
int insereAVL(Arvore *a, int valor){
    int res;

    if(a->raiz == NULL){//Arvore vazia ou nó folha;
        No *no = (No*) malloc(sizeof(No));
        if(no == NULL){//Alocacao falhou!
            return 0;
        }
        //Alocacao deu certo
        no->valor = valor;
    ...
    if(valor < atual->valor){
        if((res = insereAVL(a, valor)) == 1){//Se for = 1 a isercao funcionou, e tem q fazer o balanceamento na arvore
            if(calcularBalanceamento(atual) >= 2){
        ...
        if(valor > atual->valor){
            if((res = insereAVL(a, valor)) == 1){
                if(calcularBalanceamento(atual) >= 2){
                    if(a->raiz->dir->valor < valor){
                        rotacaoRR(a->raiz);
                    }else{
                        rotacaoRL(a->raiz);
                    }
    ...
    atual->fatorBal = maior(altura(atual->esq), altura(atual->dir)) + 1;
